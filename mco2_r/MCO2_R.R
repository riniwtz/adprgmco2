...
generate_reports <- function() {
  ...
  report2 <- flood_clean %>%
    ...
    mutate(
      ReliabilityIndex = pmin(100, (1 - (AvgDelay / 90)) * (TotalSavings / ifelse(TotalContractCost == 0, 1, TotalContractCost)) * 100),
  ...
  report3 <- report3_base %>%
    ...
    mutate(YoYChangePct = ((AvgSavings - BaselineSavings) / ifelse(abs(BaselineSavings) == 0, 1, abs(BaselineSavings))) * 100)
